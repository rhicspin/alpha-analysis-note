...
Polarization values determined from the current offline analysis currently
does not rely on any information about the \gadolinium{} peak during the
calibration procedure. The nominal detector gain \gainAm{} is
...
the fact that the bias current measurements are taken with a long period of five minutes.
...
$\approx 1.1\text{ }\mu m$. This number significantly disagrees with the value extracted
from the nominal ``banana'' fit to the carbon data where the dead layer is
estimated to be $\approx 0.15\text{ }\mu m$. A possible explanation for this discrepancy
